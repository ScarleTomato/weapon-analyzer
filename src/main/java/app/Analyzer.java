...
//					System.out.println("pickup odf: " + apOdf);
	...
	private HashMap<String, String> scanGunOdf(String odfName) throws IOException {
		...
		try {
			...
			copyProps(p, props, "leaderName", "wpnName", "isAssault", "salvoCount", "salvoDelay"
					, "altName", "aiRange", "objectClass", "shotDelay", "classLabel", "ordName", "lockRange"
					);
	...
	private void scanOrdOdf(HashMap<String, String> props, String ordOdfName) throws IOException {
		...
		try {
			...
			copyProps(p, props, "ammoCost", "xplBlast", "velocForward", "launchOrd", "lifeSpan", "shotSpeed", "lifeSpan", "shotSpeed", "lockRange"
					, "damageValue(N)", "damageValue(L)", "damageValue(H)", "damageValue(S)", "damageValue(D)", "damageValue(A)");
	...
	private void printStats(HashMap<String, String> props) {
		StringBuilder sb = new StringBuilder(props.get("wpnName"));
		sb.append("\t").append(Objects.toString(props.get("lockRange"), ""));
		sb.append("\t").append(Objects.toString(props.get("leaderRange"), ""));
		sb.append("\t").append(props.get("range"));
		sb.append("\t").append(props.get("aiRange"));
		sb.append("\t").append(props.get("odfName"));
	
		if(props.containsKey("dpsN")) {
			for(char armor : "NLHSDA".toCharArray()) {
				sb.append("\t").append(Objects.toString(props.get("dps" + armor), ""));
			}
			for(char armor : "NLHSDA".toCharArray()) {
				sb.append("\t").append(Objects.toString(props.get("dpa" + armor), ""));
			}
		}
